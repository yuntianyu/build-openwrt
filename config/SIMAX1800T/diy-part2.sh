...
sed -i "s|DISTRIB_DESCRIPTION='.*'|DISTRIB_DESCRIPTION='OpenWrt $(date +%Y.%m.%d) by nuoyan''|g" package/base-files/files/etc/openwrt_release
sed -i "s|ImmortalWrt .*|             OpenWrt $(date +%Y.%m.%d) by nuoyan|g" package/base-files/files/etc/banner
